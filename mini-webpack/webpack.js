...
class Compiler {
  ...
  run(callback) {
    ...
    const onCompiled = (err, stats, fileDependencies) => {
      ...
      //监听文件变化，如果文件发生变化会重新编译
      fileDependencies.forEach((fileDependencie) => {
        fs.watch(fileDependencie, { recursive: true }, () =>
    ...
    this.compile(onCompiled); //执行compile方法
  }
...
class Compilation {
  ...
  buildModule(name, modulePath) {
    ...
    //如果模块的路径与规则中的test匹配， 就把此规则对应的loader放到loaders数组中
    rules.forEach((rule) => {
      let { test } = rule;
  ...
  build(callback) {
    //第五步： 开始做编译工作, 根据配置文件中的“entry”配置项找到所有入口
    let entry = {};
